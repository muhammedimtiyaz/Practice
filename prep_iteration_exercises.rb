...
def factors(num)
  (1..num).find_all { |factor| num % factor == 0 }
end
...
class Array
  ...
  # p a = [1, 3, 5,7,4,98,78,23]
  # p a.bubble_sort { |num1, num2| num1 <=> num2 } # => [1,3,4,5,7,23,78,98]
  # p a
...
def substrings(string)
  ...
  string.length.times do |start_pos|
    (start_pos...string.length).each do |end_pos|
      substr = string[start_pos..end_pos]
      result << substr unless result.include?(substr)
    end
  end
...
# p [1,2,2,4,5,2].my_each{ |el| puts el }
# 1
# 2
# 2
# 4
...
# p [1,2,3].my_map { |el| el ** 2 } # => [1,4,9]
# p [1,2,3,4,5,6].my_select { |el| el.even? } #=> [2,4,6]
# p [1,2,3,4].my_inject { |acc, el| acc * el } # => 24
